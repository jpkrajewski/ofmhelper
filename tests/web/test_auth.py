"""
Covers the two-role (admin/VA) login logic on AuthMiddleware: whichever of the
...
def test_admin_password_returns_admin_role(monkeypatch):
    ...
    assert AuthMiddleware.check_password("admin-secret") == "admin"
...
def test_va_password_returns_va_role(monkeypatch):
    ...
    assert AuthMiddleware.check_password("va-secret") == "va"
...
def test_wrong_password_returns_none(monkeypatch):
    ...
    assert AuthMiddleware.check_password("something-else") is None
